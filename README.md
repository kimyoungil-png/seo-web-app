...
全工程を1ページに縦並びで表示します。左サイドバーとSERPのタブ切り替えは使用せず、SetupからFact Checkまで上から下へ順番に確認できます。完了した工程は自動的に閉じますが、アコーディオンを開いて結果を再確認・編集できます。
...
各工程の主要アクションは`Next`に統一しています。現在押すべきNextボタンと、未完了工程の入力欄には控えめなアニメーションを付けています。完了済みのNextはグレーで無効化され、必要な場合は各工程の再実行ボタンからやり直せます。
...
SuggestionはBrave Autosuggestの公式基本パラメータで取得します。

- `q`
- `country`
- `count`

Web Search用の`search_lang`を`lang`として送らず、Entity向けの`rich=true`も要求しません。Country付きリクエストが0件の場合は、Countryを外したグローバルAutosuggestを1回だけ試します。リクエスト内容と件数はSuggestionタブの`Autosuggest diagnostics`で確認できます。

AutosuggestはBrave Searchとは別のAPI機能・利用枠です。契約や権限の問題で失敗した場合は、Braveが返したエラー本文をSuggestionタブへ表示します。
...
AIのネイティブWeb検索機能は使用しません。抽出済み事実、取得済み証拠、完了済みバッチは`10-fact-check/`へ保存されるため、途中で429などが発生しても再実行時に続きから再開します。記事、AIモデル、国・言語、バッチサイズが変わった場合は、古いFact Check作業データを破棄して最初から処理します。
