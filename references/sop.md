...
- 出力言語は原則として日本語とする。
- 画面は1ページ構成で、各工程を閉じない常時表示セクションとして上から順に配置する。左サイドバー、タブ、アコーディオンは使用しない。
...
5. Suggestion
   ...
   - Autosuggestが契約プランに含まれない、または取得不能の場合は、取得済みのDiscussions、Web、Videos、Newsのタイトルから関連クエリ候補を作る
   - フォールバック成功時は生エラーを通常表示せず、分かりやすい警告と任意表示の技術診断を残す
...
- Discussions、News、Videosの個別失敗は他カテゴリを破棄せず、該当セクションにエラーを表示する。Suggestionはフォールバック成功時に生エラーを通常表示しない。
...
- Setup、SERP Research、Outline、Article Generation、Fact Checkの主要処理は、原則として1回の`Next`で生成と保存まで行う。
- OutlineとArticleを生成後に編集した場合だけ、専用のApplyボタンで保存し、後続工程をリセットする。
- Originalityは3案生成後にユーザーが1案を選ぶ必要があるため、案生成と選択確定を分ける。
- 完了ステージは緑色の`Stage complete`表示で強調する。
- 記事とFact Checkのダウンロードボタンは緑色で強調する。
- Fact Checkで503が発生した場合は一時的なモデル混雑として案内し、429は利用上限として案内する。処理済みバッチを残したままRetryできるようにする。
- Outlineの任意項目が該当しない場合は、「なし」「不要」「N/A」を出力せず、項目行自体を省略する。
